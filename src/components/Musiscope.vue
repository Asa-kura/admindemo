...
<template>
  <div class="musiscope">
    <div>
      <button @click="toggleRecord">{{ !recorder.recording ? 'start' : 'stop' }}</button>
      <button :disabled="!recorder.buffers.length" @click="recorder.clear()">clear</button>
      <button :disabled="!recorder.buffers.length" @click="playClip">play clip</button>
    </div>
    <div>
      selection:
      <button :disabled="!selection" @click="playSelection(false)">play</button>
      <button :disabled="!selection" @click="playSelection(20000)">loop 20s</button>
      <button :disabled="!selection" @click="playSelection(true)">loop</button>
      <button :disabled="!players.length" @click="stopAllPlayers()">stop all</button>
    </div>
    <wave-viewer
      :acx="acx"
    ...
    <div>
      <label>
        <input type="checkbox" v-model="passthrough" />
        <span>mic monitor</span>
      </label>
      <label>
        <span>mic gain:</span>
        <input @input="stepMicGain" type="range" :min="mic.min" :max="mic.max" value="0" />
        <span>{{ mic.gain.toFixed(1) }} dB</span>
      </label>
    </div>
  </div>
</template>
