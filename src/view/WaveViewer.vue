...
<template>
  <div class="wave-viewer">
    ...
    <Slider :value="value_ts" :min="0" :max="53" @on-input="timeSliderInput" :step="0.1"></Slider>
    <div class="handle-wave clearfix">
      <Button @click="skip(-128)" size="middle" title="快退" class="fb">&lt;&lt;</Button>
      <Button @click="skip(-16)" size="middle" title="后退">&lt;</Button>
      <Button @click="skip(16)" size="middle" title="前进">&gt;</Button>
      <Button @click="skip(128)" size="middle" title="快进">&gt;&gt;</Button>
      <Button :disabled="this.recorder.recording" @click="end = bufLenrender()" class="lb">活动</Button>
    </div>
    <div>
      <label>
        <Checkbox v-model="lockWave" />
        </Checkbox>
        <span>锁定波形图</span>
      </label>
    </div>
